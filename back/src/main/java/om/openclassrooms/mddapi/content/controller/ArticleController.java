...
public class ArticleController {
    ...
    public void createArticle(CreateArticleRequest request, @AuthenticationPrincipal User user){
        articleService.createArticle(request, user.getId());
    }

    @PostMapping("/{articleId}/comments")
    @ResponseStatus(HttpStatus.CREATED)
    public void createCommentForArticle(@RequestParam @NotEmpty String comment, @PathVariable Long articleId, @AuthenticationPrincipal User user){
        commentService.createCommentForArticle(articleId, comment, user.getId());
    }
